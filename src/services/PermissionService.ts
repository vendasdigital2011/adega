...
export class PermissionService extends BaseService {
  ...
  public async listAll(): Promise<Permission[]> {
    try {
      ...
      this.handleError(error, "permissions.list_all")
    }
  }
  ...
  public async grant(roleId: string, permissionId: string): Promise<void> {
    try {
      ...
      if (this.isOfflineOrDemoMode(error)) {
        const list = this.getLocalMockStore<string>(`role_permissions_${roleId}`, [])
        if (!list.includes(permissionId)) {
          list.push(permissionId)
          this.saveLocalMockStore(`role_permissions_${roleId}`, list)
        }
        return
      }
  ...
  public async revoke(roleId: string, permissionId: string): Promise<void> {
    try {
      ...
      if (this.isOfflineOrDemoMode(error)) {
        const list = this.getLocalMockStore<string>(`role_permissions_${roleId}`, [])
        const newList = list.filter((id) => id !== permissionId)
        this.saveLocalMockStore(`role_permissions_${roleId}`, newList)
        return
      }
